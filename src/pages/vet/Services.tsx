...
export default function VetServices() {
  ...
  const all = toArray(services.data);

  const [showForm, setShowForm] = useState(false);
  const [editing, setEditing] = useState<string|null>(null);
  const [form, setForm] = useState({ service_name:'', service_type:'consultation', description:'', price:'', duration_minutes:'30', is_mobile:false, region:'' });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');

  const resetForm = () => { setForm({ service_name:'', service_type:'consultation', description:'', price:'', duration_minutes:'30', is_mobile:false, region:'' }); setEditing(null); setShowForm(false); setError(''); };
  ...
  const handleEdit = (s: typeof all[0]) => {
    setForm({ service_name: s.service_name, service_type: s.service_type, description: s.description, price: s.price, duration_minutes: String(s.duration_minutes), is_mobile: s.is_mobile, region: s.region });
        ...
        <Card style={{ marginBottom:'var(--sp-md)', border:'1px solid var(--col-primary)' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'var(--sp-sm)' }}>
            ...
            <div>
              ...
              <select className="form-input" value={form.service_type} onChange={e => setForm(f => ({ ...f, service_type: e.target.value }))}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:'var(--sp-sm)', paddingTop:20 }}>
              <input type="checkbox" id="is_mobile" checked={form.is_mobile} onChange={e => setForm(f => ({ ...f, is_mobile: e.target.checked }))} />
            ...
            <div style={{ gridColumn:'1/-1' }}>
              ...
              <textarea className="form-input" rows={2} placeholder="Describe the service…" style={{ resize:'vertical' }} {...inp('description')} />
            </div>
          </div>
